...
sys.path[0] = 'C:/.../PythonClient/Course1FinalProject'
...
fieldnames = ['Simulation_time','speed','accelerometer_X','accelerometer_Y','accelerometer_Z','yaw','roll','pitch','steer',
              'location_X','location_Y','estimated_location_X','estimated_location_Y',
              'error_x','error_y','error_x_new','error_y_new']  
df = pandas.DataFrame(columns=fieldnames)
...
def get_current_pose_2(measurement):
    ...
    acceleration = measurement.player_measurements.acceleration
    
    return (roll, pitch, acceleration)
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        ekf.initialize_with_gnss(-184.0, 80.0, 1.0)
        last_x = -184.0
        last_y = 80.0
        ...
        AVMon = tf.keras.models.load_model('AVMon.model')
        ###############################################################
        ###############################################################
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            current_roll, current_pitch, accelerometer = \
                get_current_pose_2(measurement_data)
            ...
            controller.update_controls(P = kp, I = ki, D = kd)
            cmd_throttle, cmd_steer, cmd_brake = controller.get_commands()

            ###############################################################
            ###############################################################
            gyroscope_x = current_roll
            gyroscope_y = current_pitch
            gyroscope_z = current_yaw
            current_z = 1.0
   
            # EKF prediction
            ekf.predict_state_with_imu(accelerometer.x, accelerometer.y, accelerometer.z, gyroscope_x, gyroscope_y, gyroscope_z)

            # EKF correction
            ekf.correct_state_with_gnss(current_x, current_y, current_z)
                       
            # Get estimated parameters
            estimated_location = ekf.get_location()
            
            measurements_in  = np.column_stack((current_speed, accelerometer.x, accelerometer.y, accelerometer.z,current_yaw,cmd_steer))
            #measurements_in.reshape(1, 12)
            #measurements_in.reshape(1, -1)
            sample_in        = scaler_in.transform(measurements_in)
            predictions      = AVMon.predict(sample_in)
            estimated_errors = scaler_out.inverse_transform(predictions) 

            if current_timestamp < 8.0:
                x_error = 0.0
                y_error = 0.0
                error_x_new = 0.0
                error_y_new = 0.0
            else:
                x_error = abs(estimated_location[0] - last_x)
                y_error = abs(estimated_location[1] - last_y)
                error_x_new = x_error - abs(estimated_errors[0][0])
                error_y_new = y_error - abs(estimated_errors[0][1])
            ...
            #print('----------------------------------------------------------------')
            df.loc[row, 'Simulation_time'] = current_timestamp 
            df.loc[row, 'speed'] = current_speed   
            df.loc[row, 'accelerometer_X'] = accelerometer.x
            df.loc[row, 'accelerometer_Y'] = accelerometer.y
            df.loc[row, 'accelerometer_Z'] = accelerometer.z
            df.loc[row, 'yaw'] = current_yaw
            df.loc[row, 'roll'] = current_roll
            df.loc[row, 'pitch'] = current_pitch         
            df.loc[row, 'steer'] = cmd_steer                    
            df.loc[row, 'location_X'] = current_x
            df.loc[row, 'location_Y'] = current_y
            df.loc[row, 'estimated_location_X'] = estimated_location[0]
            df.loc[row, 'estimated_location_Y'] = estimated_location[1]
            df.loc[row, 'error_x'] = x_error
            df.loc[row, 'error_y'] = y_error
            df.loc[row, 'error_x_new'] = error_x_new
            df.loc[row, 'error_y_new'] = error_y_new
            row = row + 1
            #print('----------------------------------------------------------------')
...
def main():
    ...
    while True:
        try:
            ...
            print('Done.')
            df.to_csv('Carla_Logging.csv')
            print('Carla_Logging is saved')
            return
