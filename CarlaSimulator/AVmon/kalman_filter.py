...
class ExtendedKalmanFilter:
    ...
    def initialize_with_gnss(self, gnss_x, gnss_y, gnss_z, samples_to_use=10):
        ...
        if self.gnss_init_xyz is None:
            self.gnss_init_xyz = np.array([gnss_x, gnss_y, gnss_z])
        else:
            self.gnss_init_xyz[0] += gnss_x
            self.gnss_init_xyz[1] += gnss_y
            self.gnss_init_xyz[2] += gnss_z
        self.n_gnss_taken += 1
    ...
    def get_velocity(self):
        """Return the estimated vehicle velocity
        :rtype: list
        """
        return self.v.reshape(-1).tolist()
    
    def get_orientation(self):
        """Return the estimated vehicle orientation
        :rtype: list
        """
        return self.q.reshape(-1).tolist()

    def predict_state_with_imu(self, accelerometer_x, accelerometer_y, accelerometer_z, gyroscope_x, gyroscope_y, gyroscope_z):
        ...
        imu_f = np.array([accelerometer_x, accelerometer_y, accelerometer_z]).reshape(3, 1)
        imu_w = np.array([gyroscope_x, gyroscope_y, gyroscope_z]).reshape(3, 1)

        # IMU sampling time
        delta_t = 0.1
        #delta_t = current_time - self.last_ts
    ...
    def correct_state_with_gnss(self, gnss_x, gnss_y, gnss_z):
